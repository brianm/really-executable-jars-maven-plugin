...
public class ReallyExecutableJarMojo extends AbstractMojo {
    ...
    private void makeExecutable(File file)
        ...
        try {
            ZipPrefixer.validateZipOffsets(target); // optional, just an extra safety

            final long prefixLength = ZipPrefixer.applyPrefixes(target, getPreamble(target.toUri()), "\n\n".getBytes(UTF_8));
            ZipPrefixer.adjustZipOffsets(target, prefixLength);
        } catch (IOException e) {
